...
function AddExpenseButton({ appraisal, active = true, clientId }: AddExpenseProps) {
  ...
  const [create, { loading }] = useCreate('appraisal_expense');
    ...
    create(
      ...
      },
      {
        onSuccess: () => {
          setShowDialog(false);
          setDisabled(false);
          notify('appraisal_expense.created');
          refresh();
        },
        onFailure: ({ error }: any) => {
          setDisabled(false);
  ...
  return (
    <>
      <Button onClick={handleClick} label="ADD EXPENSE" disabled={!active}>
        <IconContentAdd />
      </Button>
      <Dialog fullWidth={true} maxWidth={'md'} open={showDialog} onClose={handleCloseClick} aria-label="Add Expense">
        <FormWithRedirect
          initialValues={{
          ...
          render={({ handleSubmitWithRedirect, saving, ...rest }: any) => (
            <DialogContent classes={{ root: popUpClasses.dialogContent }}>
              <Card variant="outlined" className={popUpClasses.formBottom}>
                <CardHeader title="Add Expense" classes={{ root: popUpClasses.cardHeader }}></CardHeader>
                <Divider></Divider>
                <Box className={popUpClasses.formContainer}>
                  ...
                  <Typography classes={{ root: popUpClasses.heading }}>EXPENSE</Typography>
                  <Divider classes={{ root: popUpClasses.divider }}></Divider>
